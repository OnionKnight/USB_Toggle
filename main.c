...
int main(int argc, char **argv) {
	...
	bool ret;
	u8 devcount;
	u32 sector_size, sectors;
	u8 *mbr;
	u8 retry = 5;
	usb_device_entry *usbdev;
	...
	while(retry)
	{
	usbdev = Get_Device_List(&devcount);
	if(devcount > 0){break;}
	else { retry--;}
	}
	...
	{
		ret = Mount_Storage(usbdev[i].device_id,usbdev[i].vid,usbdev[i].pid);
			if(!ret){ printf("Mount Failed"); exit(0);}
